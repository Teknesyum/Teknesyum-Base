...
kontrol(
  typeof T.meta === 'object' && typeof T.meta.neden === 'string' && T.meta.neden.length > 0,
  'meta.neden yok'
);
for (const grup of ['marka', 'rol', 'turetilmis', 'sure', 'easing', 'font']) {
  ...
  for (const [ad, t] of Object.entries(T[grup] || {})) {
    kontrol(
      typeof t.neden === 'string' && t.neden.length > 0,
      grup + '.' + ad + ' neden alani tasimiyor'
    );
  }
}
...
for (const [ad, t] of Object.entries(T.rol || {})) {
  const degerli = t.deger !== undefined,
    refli = t.ref !== undefined;
  kontrol(
    degerli !== refli,
    'rol.' + ad + ' ya deger ya ref tasimali, ikisi birden ya da hicbiri degil'
  );
}
for (const [ad, t] of Object.entries(T.turetilmis || {})) {
  kontrol(
    t.deger === undefined,
    'turetilmis.' + ad + ' hesaplanmis deger tasiyor; bildirimsel olmali (ref/tabanlar + alpha)'
  );
}
...
uretilenler.forEach((d, i) => {
  kontrol(
    once[i].equals(birinci[i]),
    d + ' depodaki halinden farkli uretildi — elle mi duzenlendi, JSON mu kaydi?'
  );
});
...
try {
  const diff = cp
    .execSync(
      'git diff --numstat -- ' +
        uretilenler.map((d) => '"teknesyum/skills/teknesyum-ui/assets/' + d + '"').join(' '),
...
const aranacaklar = {
  ...
  'Theme.axaml': ['ANLAMSAL ROL KATMANI', 'Bu firca tek basina kullanilmaz', 'bilerek yok'],
  'Palette.cs': [
    '`Info` BİLEREK YOK',
    'ROL KAZANIR',
...
for (const [dosya, listeler] of Object.entries(aranacaklar)) {
  for (const parca of listeler) {
    kontrol(
      icerikler[dosya].includes(parca),
      dosya + ' gerekce yorumunu kaybetti: ' + JSON.stringify(parca)
    );
  }
}

kontrol(
  icerikler['theme.css'].includes('@theme') && icerikler['theme.css'].includes(':root'),
  'theme.css iki katmani da tasimiyor'
);
