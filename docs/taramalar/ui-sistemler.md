...
Mercek: büyük kuruluşların hareket ve hiyerarşi kuralları. Sayılar depo kaynak
dosyalarından okundu; okunamayan yer "doğrulanamadı" işaretli.
...
| Apple HIG | depo yok | — | — | — | tescilli |

`primer/design` arşiv; canlı hareket token'ları `primer/primitives` altında.

## 1 · Alanın bugün yaptığı

**Material 3 — 16 süre, 10 easing.** `tokens/versions/v0_192/_md-sys-motion.scss`:
`duration-short1..4` 50/100/150/200 ms, `medium1..4` 250/300/350/400,
`long1..4` 450/500/550/600, `extra-long1..4` 700/800/900/1000. Easing üç aile × üç yön:
`standard`, `standard-accelerate`, `standard-decelerate`; aynısı `emphasized` ve `legacy`
için; artı `linear`. `easing-emphasized` = `cubic-bezier(0.2, 0, 0, 1)` — standardın
`--tk-e-out` değeriyle **birebir aynı**.
...
· 0.9/300); effects: 1.0/1600 · 1.0/3800 · 1.0/800, **iki şemada da aynı**.
Yani expressive'in tamamı `spatial` eksenindeki damping düşüşünden (0.9 → 0.6-0.8) ibaret;
M3 "expressive" derken renk/opaklığa dokunmuyor, yalnız konum hareketine sekme ekliyor.
`material-web` web token'larında **spring yok** (`versions/latest/_md-sys-motion.scss`
404); expressive şu an Compose tarafında.
...
**Primer — iki katman, üstelik LLM notlu.** `base/duration` 0-1000 ms arası 13 basamak.
Üstünde `functional/motion`: süre `micro` 100, `short` 200, `medium` 300, `long` 500;
...
isteğe bağlı olsun** — bilgiyi yalnız hareketle iletme; geri bildirim kısa ve kesin olsun;
**sık tekrarlanan etkileşimlere hareket ekleme**; ve **"Let people cancel motion"** —
kullanıcıyı animasyon bitene kadar bekletme. Sayısal karşılık HIG'de değil SwiftUI'da:
`Animation.smooth` temel sekme 0, `.snappy` 0.15, `.bouncy` 0.3 (Apple SwiftUI belgeleri;
varsayılan süre değeri belgede sayı olarak görünmedi — doğrulanamadı).
...
eşleştirmeyi yazana bırakıyor — yanlış çift (`--tk-t-slow` + `--tk-e-in`) mümkün.
**Alınmalı — evet.** Dört reçete yeter: hover, durum değişimi, giriş, çıkış. §5.4.

**c. Çıkış girişten kısa değil.** Primer `exit` = 200, `enter` = 300; gerekçe token
dosyasında yazılı ("Shorter than enter to feel snappy"). Standart bu ayrımı söylemiyor.
**Alınmalı — tek satır kural.** §5.4 mikro etkileşim tablosuna satır.
...
`assets/theme.css` içinde her süre token'ının yanına tek satır "nerede" notu. §5.4 / assets.
...
sekmeyi **yalnız `spatial`** eksende artırıyor, `effects` ekseninde üç hızda da damping 1.0
— renk/opaklık hiçbir zaman sekmiyor. Standardın "spring yalnızca basma" kuralı bu ayrımın
daha dar ama aynı yönde hali. Yay eksenini genel açmak iptal edilebilirlikle de çelişir:
CSS'te yay `linear()` ile örneklenmiş sabit eğridir, yarıda kesilince hızı korumaz.

**M3 "expressive" ile "süs değil geri bildirim" ilkesi çelişmiyor.** Expressive daha uzun
ya da daha çok animasyon demiyor; değişen şey miktar değil karakter — damping düşüyor,
süre artmıyor. Standardın yasakladıkları (sonsuz döngü, her render'da giriş animasyonu,
40 DIP uçuş) M3 expressive'de de yok.

**360 ms tavanı savunulabilir.** M3'ün 1000, Carbon'un 700, Fluent'in 500 ms basamağı var
ama üçü de bunları tam ekran/hero geçişi için tanımlıyor (Carbon `slow-02`: "background
dimming, large hero transitions"). Primer'in kendi kuralı standartla aynı yerde: "MUST keep
...
- `gh api repos/{material-components/material-web, microsoft/fluentui, carbon-design-system/carbon, primer/design, primer/primitives}` ve `/releases/latest` — 2026-08-22
- material-web `tokens/versions/v0_192/_md-sys-motion.scss`
- androidx-main `compose/material3/.../MotionScheme.kt`, `tokens/{MotionTokens, ExpressiveMotionTokens, StandardMotionTokens}.kt`
...
- carbon `packages/motion/src/{dtcg/motion.json, dtcg/surfaces.json, tokens.ts, surfaces.ts, package.json}`
- primer/primitives `src/tokens/base/motion/{easing,timing}.json5`, `src/tokens/functional/motion/motion.json5`
- developer.apple.com HIG "Motion" (tutorials/data JSON); SwiftUI `Animation.smooth/snappy/bouncy`
- Standart: `teknesyum/skills/teknesyum-ui/SKILL.md` §5.4, `assets/theme.css`
