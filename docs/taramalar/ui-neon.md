...
1. **Katmanlı glow.** react-bits `ElectricBorder.css`: üç `absolute` katman — `.eb-glow-1`
   `filter: blur(1px)`, `.eb-glow-2` `blur(4px)`, `.eb-background-glow` `blur(32px)` +
   `scale(1.1)` + `opacity:.3`. **Blur statik, animasyonlanmıyor.**
...
3. **Gürültü dokusu — iki karşıt uygulama.** cult-ui `texture-overlay.tsx` on iki dokuyu
   ...
   aynı görüntüyü `requestAnimationFrame` döngüsünde `createImageData` + `Math.random()`
   ile her karede üretiyor (34-53).
...
5. **Blur katmanı.** magicui `progressive-blur.tsx`: sekiz kademeli `backdropFilter: blur()`
   (`[0.5,1,2,4,8,16,32,64]`), her katman `maskImage` ile %12,5'lik dilime kırpılıyor. Statik.
...
**`box-shadow`** → `pulse`, `pulse-ripple` (`pulsating-button` bunu kullanıyor);
**`filter: blur()`** → `blur-fade.tsx:55-60`, girişte `blur(6px)→blur(0px)`, 400 ms.
Buna karşılık `meteor`, `marquee`, `spin-around`, `ripple`, `ping`, `shimmer-slide` yalnız
`transform`/`opacity`. Yani depo tutarsız — kural yok, tercih var.
...
and `opacity` only"*), 6. madde birebir bizim keyframe gerekçemiz (*"interruptible …
not keyframes that restart from zero"*). İlginç olan: **depo bu standardı yazmış, kendi
...
**Geçiş > keyframe korunmalı.** Gerekçe artık bağımsız doğrulanmış: react-bits kendi
STANDARDS.md'sinde aynı sonuca aynı gerekçeyle varmış. Alanın çoğunluğu keyframe kullanıyor;
kuralı yazanlar keyframe'in yanlış olduğunu kabul ediyor.

**Yalnız `transform`/`opacity` korunmalı.** Aynı şekilde doğrulandı (STANDARDS 7. madde).
Fark uygulamada: magicui'nin dokuz tokenı ve react-bits'in kendi bileşenleri kuralı
çiğniyor. Kural doğru, disiplin eksik — standardımızın değeri o disiplini zorunlu kılmasında.
...
**Metne glow verilmemesi korunmalı.** magicui'de metne efekt veren en az beş bileşen var
...
**360 ms tavanı korunmalı.** react-bits bağımsız olarak daha sıkısını söylüyor: "UI
animations stay under 300ms". 360 ms'yi yalnız sayfa/görünüm değişimine ayırmamız onların
"modals, drawers 200–500ms" aralığıyla uyumlu.
...
| ease-in-out | yok | `cubic-bezier(0.77, 0, 0.175, 1)` | — |
| çekmece/panel eğrisi | yok | `cubic-bezier(0.32, 0.72, 0, 1)` | — |
| basma | `scale(0.98)`, 90 ms | `scale(0.97)`, 160 ms ease-out | `active:translate-y-px`, 300 ms |
| tooltip | 160 ms | 125–200 ms | — |
| açılan menü, select | 160 ms | 150–250 ms | — |
| diyalog, panel | 240 ms | 200–500 ms | — |
| üst sınır | 360 ms | 300 ms (modal hariç) | tavan yok |
| kademe | 40 ms, en çok 6 | 30–80 ms | — |
| spring | `cubic-bezier(0.34,1.36,0.64,1)`, yalnız basma | `{duration:.5, bounce:.2}` · `{mass:1, stiffness:100, damping:10}` | — |
| giriş kayması | 8 DIP | `translateY(8px)` | `offset: 6` (blur-fade) |
| giriş ölçeği | belirtilmemiş | `scale(0.9–0.97)`; `scale(0)` yasak | — |
...
- **magicui — MIT.** `LICENSE.md` düz MIT, ek şart yok, ayrı marka koruma metni yok.
  Kopyalanabilir; §5.6 gereği `docs/licenses.md`'ye satır yazılır.
- **cult-ui — MIT** (Copyright 2023 Jordan-Gilliam). Kopyalanabilir. Not: sitede ayrıca
  ücretli "cult pro" kataloğu var (`get-cult-pro-cta.tsx`); MIT yalnız açık bileşenleri kapsar.
...
- **react-bits — MIT + Commons Clause v1.0. OSI onaylı değil.** GitHub API `NOASSERTION`
  ...
  *"you do not sell, sublicense, or redistribute the components themselves — whether alone,
  in a bundle, or as a ported version."* **Bizim için sonuç:** react-bits'ten alınan bir
  efekti `teknesyum-ui` standardının parçası yapıp başka projelere dağıtmak "ported version"
  sayılabilir. §5.6 madde 2 uygulanır — **fikir alınır, kod alınmaz.** Depoda ayrıca ücretli
  katalog var (`Pro.js`: `PRO_COUNTS.total = 683` — deponun kendi beyanı, bağımsız
  doğrulanamadı); ücretli içerik lisansın da dışında.
...
- react-bits: `AGENTS/SKILLS/review-animations/{SKILL.md,STANDARDS.md}`, `src/content/Animations/{ElectricBorder,GlareHover,StarBorder,Noise}/`, `src/constants/Pro.js`, `package.json`, `LICENSE.md`
- cult-ui: `apps/www/public/registry/styles/default/texture-overlay.json`, `LICENSE.md` · hyperui: `src/content/blog/animated-border-gradient-with-tailwindcss.mdx`, `package.json`, `LICENSE`
