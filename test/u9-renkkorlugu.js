...
function hexYap(rgb) {
  return '#' + rgb.map(v => Math.round(kirp(v) * 255).toString(16).padStart(2, '0')).join('');
}
...
const IZDUSUM_PROTAN = [[0, 2.02344, -2.52581], [0, 1, 0], [0, 0, 1]];
const IZDUSUM_DEUTAN = [[1, 0, 0], [0.494207, 0, 1.24827], [0, 0, 1]];
...
function rgbXyzMatrisi() {
  const [xr, yr] = [0.6400, 0.3300];
  const [xg, yg] = [0.3000, 0.6000];
  const [xb, yb] = [0.1500, 0.0600];
  const [xw, yw] = [0.3127, 0.3290];
  const M = [
    [xr / yr, xg / yg, xb / yb],
    [1, 1, 1],
...
function deltaE2000(lab1, lab2) {
  ...
  const hp1 = aci(b1, ap1), hp2 = aci(b2, ap2);
...
function hesapla() {
  ...
  for (const [ad, hex] of Object.entries(PALET)) {
    simTablo[ad] = { hex, protanopi: simule(hex, 'protanopi'), deuteranopi: simule(hex, 'deuteranopi') };
  }
...
function katmanA() {
  ...
  for (const [ad, M] of [['protan', VIENOT_PROTAN_YAYIN], ['deutan', VIENOT_DEUTAN_YAYIN]]) {
    // Satır toplamı 1: beyaz beyaz kalır.
    for (let i = 0; i < 3; i++) yakin(`A2 ${ad} satır ${i} toplamı`, M[i].reduce((s, v) => s + v, 0), 1, 1e-5);
...
function belgeSatirlari(metin, baslik) {
  ...
  return bolum.split('\n')
    ...
    .map(l => l.trim().slice(1, -1).split('|').map(h => h.trim()))
    .slice(1); // başlık satırı
}
const say = s => Number(String(s).replace(/\*\*/g, '').replace(',', '.').replace(/[^0-9.\-]/g, ''));
...
function katmanB(h) {
  ...
  if (s1) {
    onay('B1 satır sayısı', s1.length === Object.keys(PALET).length, `${s1.length} satır, ${Object.keys(PALET).length} bekleniyordu`);
    for (const satir of s1) {
      ...
      onay(`B1 ${ad} protanopi`,    hx(satir[2]) === b.protanopi,   `belge ${hx(satir[2])}, hesap ${b.protanopi}`);
      onay(`B1 ${ad} deuteranopi`,  hx(satir[3]) === b.deuteranopi, `belge ${hx(satir[3])}, hesap ${b.deuteranopi}`);
    }
  }
  ...
  if (s2) {
    onay('B2 satır sayısı', s2.length === CIFTLER.length, `${s2.length} satır, ${CIFTLER.length} bekleniyordu`);
    for (const satir of s2) {
      const ad = satir[0].replace(/[`*]/g, '');
      const b = h.ciftTablo.find(c => c.ad === ad);
      onay(`B2 ${ad} tanınıyor`, !!b);
      if (!b) continue;
      onay(`B2 ${ad} hexler`, hx(satir[1]) === b.a && hx(satir[2]) === b.b,
        `belge ${hx(satir[1])}/${hx(satir[2])}, hesap ${b.a}/${b.b}`);
      TIPLER.forEach((t, i) => {
        yakin(`B2 ${ad} ${t} ΔE`, say(satir[3 + i]), Number(b[t].dE.toFixed(1)), 0.05);
      });
      onay(`B2 ${ad} yorum`, satir[6].replace(/[`*]/g, '') === yorum(Math.min(...TIPLER.map(t => b[t].dE))),
  ...
  if (s3) {
    for (const satir of s3) {
      ...
      onay(`B3 ${ad}/${tip} hex`, hx(satir[2]) === b[tip].hex, `belge ${hx(satir[2])}, hesap ${b[tip].hex}`);
